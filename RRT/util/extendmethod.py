...
def extend_with_rewire(map_info: MapInfo, tree: RRT, root: NDArray[Any], new_node_info: NDArray[Any], radius: float, smooth_method = 'line') -> RRT:
    neighbors = tree.get_nearest_neighbors(new_node_info, radius, condition=lambda x: x <= radius)
    ...
    for neighbor in neighbors:
        ...
        if length < min_val and map_info.is_feasible(tree.get_route(target=neighbor_info).append(new_node_info), method=smooth_method):
            min_val = length
            min_id = neighbor
    if min_id == -1:
        return
